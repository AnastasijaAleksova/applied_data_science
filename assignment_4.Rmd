...
```{r}
library(modelsummary); library(tidyverse); library(dplyr)
treat <- if_else(GrowthSW$revolutions > 0, "More than 0 revolutions", "No revolutions") 
  datasummary(growth + rgdp60 ~ mean + median + sd + min + max, data = GrowthSW)

  treat_growth <- summarise(GrowthSW,
                      mean = mean(growth),
                      median = median(growth),
                      sd = sd(growth),
                      min = min(growth),
                      max = max(growth))
  rownames(treat_growth) <- c("growth")
  
  treat_rgdp60 <- summarise(GrowthSW,
                              mean = mean(rgdp60),
                              median = median(rgdp60),
                              sd = sd(rgdp60),
                              min = min(rgdp60),
                              max = max(rgdp60))
  rownames(treat_rgdp60) <- c("rgdp60")
  
  treat_table <- rbind(treat_growth, treat_rgdp60)
```
